...
const getProductCategories = () => {
    const API_ENDPOINT = "../api/product-categories.php";
...
const handleProductUpload = event => {
    ...
    const API_ENDPOINT = "../api/products.php";
    const request = new XMLHttpRequest();
...
const deleteProduct = vehicleId => {
    ...
    const API_ENDPOINT = "../api/transport-vehicles.php";
    const request = new XMLHttpRequest();
...
const updateProducts = products => {
    ...
    products.forEach((product, index) => {
        ...
        if( product.varieties &&
            ...
            url = "../api" + product.varieties[0].images[0].path;
            alt = product.name;
        } else {
        ...
        if(element.images)
            element.images.forEach((element2, index2) => {
                if(element2.id == undefined){
                    ...
                    varietyElements[index].getElementsByClassName("variety-image")[index2].src = "../api" + element2.path;
                }
            });
    });
}
